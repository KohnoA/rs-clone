      ...
      <div className={styles.recipeInfoPart}>
        ...
        <div className={styles.infoWrapper}>
          <div style={{ width: '100%' }} className={styles.ingredients}>
            ...
            <ul>
              {props.ingredientLines?.map((line, i) => (
                <li key={i}>{line}</li>
              ))}
            </ul>
          </div>
          <div className={styles.typesWrapper}>
            <div>
              <h3>Cuisine</h3>
              <ul>
                {props.cuisineType?.map((cuisine, i) => (
                  <li className={styles.mainInfo__item} key={i}>
                    {cuisine}
                  </li>
                ))}
              </ul>
          ...
          <div className={styles.nutrient__info}>
            <div className={styles.labels}>
              ...
              <ul className={styles.healthWrapper}>
                {props.healthLabels?.map((health, i) => (
                  <li className={styles.health__item} key={i}>
                    {health}
                  </li>
                ))}
              </ul>
            ...
            <div className={styles.labels}>
              ...
              <ul>
                <li>
                  <span className={styles.nutrient__header}>Calories</span>{' '}
                  <span className={styles.nutrient__total}>{props.calories?.toFixed(2)}</span> kcal
                </li>
